...
# from .....ops.pointnet2.pointnet2_batch.pointnet2_utils import *
...
# input_data = torch.randn(7000, 32, 4).cuda()
# net = EncP(in_channels=4, input_points=1024, num_stages=4, embed_dim=36, k_neighbors=40, beta=100, alpha=1000, LGA_block=[2,1,1,1], dim_expansion=[2,2,2,1], type='scan').cuda()
...
# 创建随机输入数据
xyz = torch.randn(1, input_points, in_channels).cuda()  # 三维空间坐标
features = torch.randn(1, input_points, in_channels).cuda()  # 点特征

# 创建模型实例
model = EncP(in_channels=in_channels, input_points=input_points, num_stages=num_stages, embed_dim=embed_dim,
             k_neighbors=k_neighbors, alpha=alpha, beta=beta, LGA_block=LGA_block, dim_expansion=dim_expansion,
             type=type).cuda()
