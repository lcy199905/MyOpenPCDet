...
class MAFFNoPool(nn.Module):

    def __init__(self, channels=32):
        super(MAFFNoPool, self).__init__()
...
class MAFF(nn.Module):
    ...
    def forward(self, x):
        ...
        # 1, N, C -> 1, C, N
        xa = x.permute(0, 2, 1)
        xl = self.swish_att(xa).permute(0, 2, 1)
        # 1, N, C -> N, C
        xl = xl.squeeze(0).contiguous()
        # 1, C, N -> 1, C, 1
        xg_pool = self.globalAvgPool(xa)
        xg = self.reLU_att(xg_pool).permute(0, 2, 1)
        # 1, C
        xg = xg.squeeze(0).contiguous()
        xlg = xl + xg
        # xlg = self.norm(xlg)
        wei = self.sigmoid(xlg)
...
#         return xi
    ...
    #     # 1, N, C -> N,C
    #     xl = xl.squeeze(0).contiguous()
    #     # 1, N, C -> 1, C, N -> 1, C, 1 -> 1, 1, C
    #     xg_pool = self.globalAvgPool(xa.permute(0, 2, 1)).permute(0, 2, 1)
    #     # 1, C
    #     xg_pool = xg_pool.view(xg_pool.size(0), -1)
    #     xg = self.reLU_att(xg_pool)
    #     xlg = xl + xg
    #     # wei = self.sigmoid(xlg)
    #     wei = self.softmax(xlg)
    #
    #     # xi = input_feature * (1 + wei)
    #     xi = input_feature * wei
    #     return xi

class PFNLayerV3(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 use_norm=True,
        ...
        if not self.last_vfe:
            ...
            self.maff1 = MAFFNoPool(out_channels)
    ...
    def forward(self, inputs, unq_inv):
        ...
        if self.last_vfe:
            ...
            # x_mean = torch_scatter.scatter_mean(x, unq_inv, dim=0)
            x_concatenated = x_max + x_sum + x_mean

            # return torch.cat([x_concatenated, x_max], dim=1)
            # return x_concatenated / 2.0
...
class DynamicPillarVFESimple2D(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, grid_size, point_cloud_range, **kwargs):
        ...
        # self.use_relative_xyz = self.model_cfg.get('USE_RELATIVE_XYZ', False)
